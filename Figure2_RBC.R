## Script to generate manuscript figure 2 (RBC)
## Alexander Okamoto
## June 15, 2023
...
library(ggforce) #for plotting circle
...
#create red blood cell image
blood_cell_coords <- data.frame(XPOS =c(50, 50, 50), 
                                YPOS = c(50, 45, 50), 
                                radius = c(50, 32, 30), 
                                names = c("inner", "middle","outer"))

background_plot <- data.frame(x = 1:100, y = 1:100)
blood_cell_plot <- ggplot(background_plot, aes(x = x, y = y)) + 
  geom_blank() + 
  geom_circle(data = blood_cell_coords, aes(x0 = XPOS, y0 = YPOS, y = YPOS, r = radius, fill = names, color = names)) + 
  scale_fill_manual(values=c("firebrick2", "indianred3", "firebrick4"))+
  scale_color_manual(values=c("firebrick2", "indianred3", "firebrick4")) +
...
ggsave(filename = "RBC_picture.jpeg", device = "jpeg", plot = blood_cell_plot, path = "~/Polygenic_Inference/Big_Loop_Output2", width = 11, height = 11, units = "cm", dpi = 1200)
blood_cell_jpeg <- rasterGrob(jpeg::readJPEG("~/Polygenic_Inference/Big_Loop_Output2/RBC_picture.jpeg"))

#generate plot
PGLS_plot_mam <- ggplot(trait_results_mammals, aes(x=trait_rank, y=pos_prop_rank, shape = Group)) + 
  annotation_custom(blood_cell_jpeg, xmin=-2, xmax=4, ymin=-4, ymax=4) +
  annotation_custom(blood_cell_jpeg, xmin=118, xmax=126, ymin=4, ymax=12) + 
  annotation_custom(blood_cell_jpeg, xmin= 116, xmax=122, ymin=-4, ymax=4) + 
  annotation_custom(blood_cell_jpeg, xmin=122, xmax=128, ymin=-4, ymax=4) +
  geom_point(color="blue", size = 0.5, stroke = 0.2)  + 
  labs(x = "RBC Rank", y = "Score Rank \n (Mammals)") + 
...
PGLS_plot_mam + annotation_custom(blood_cell_jpeg, xmin= -2, xmax= 0, ymin=-0.2, ymax=-1) +
  coord_cartesian(clip = "off")
...
#plot results 
#plot results 
CC_plot <- ggplot(data = CC_results, aes(x= trait_rank, y = pos_prop_rank)) + geom_point(size = 0.5, stroke = 0.2) + 
  labs( x = "RBC Rank", y= "Score Rank \n (CC mice)") + 
...
monoPlot <- ggplot(phylo_results_df, aes(y = abs_trait_change, x = residual_sign)) + geom_boxplot(fill = "gray", linewidth = 0.1, outlier.size = 0.0001) + 
  ...
  annotate(geom="text", x=1.5, y=35, label=paste("Wilcoxon-rank sum, P < ", round(clades_wilcox_corr$p.value, digits = 3), sep = ""),
           color="black", size = 6/.pt)


overlap <- traits_df$SNP_n_prim[i] - traits_df$SNP_n_prim_uniq[i]
# Input in the form of a named numeric vector
fit1 <- euler(c("A" = traits_df$SNP_n_prim[i]-overlap, "B" = traits_df$SNP_n_mam[i]-overlap, 
                "A&B" = overlap))

jpeg(file="~/Polygenic_Inference/RBC_venn.jpeg", width = 1000, height = 500)

plot(fit1, quantities = TRUE, labels = F, fills = c("pink", "cornflowerblue"))
#grid::grid.text("Residual variance: 80.8%", x=0.83, y=0.12, gp=gpar(col="black", fontsize=18, fontface=1))

dev.off()

venn_jpeg <- rasterGrob(jpeg::readJPEG("~/Polygenic_Inference/RBC_venn.jpeg"))

#make complete plot with phylopics
background_plot <- data.frame(x = 1:100, y = 1:50)
venn_plot <- ggplot(background_plot, aes(x = x, y=y )) + geom_blank() + annotation_custom(venn_jpeg, xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf) + 
  add_phylopic(uuid="8a187391-82a3-4d9b-a402-3a310bf7dc38", x= 10, y= 25, ysize=15, alpha = 1, color = "red") + 
  add_phylopic(uuid="2a070808-0874-409d-81c0-d0c1995e54bb", x= 92, y= 25, ysize=15, alpha = 1, color = "blue") + 
  theme_bw() +
  theme(panel.border = element_blank(), 
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
  ...
  annotate(geom="text", x=10, y=38, label="Primate Conserved\nSNP Positions",
           color="black", size = 2) + 
  annotate(geom="text", x=92, y=38, label="Mammal Conserved\nSNP Positions",
           color="black", size = 2) + 
  geom_segment(lineend = "butt", linejoin = "mitre", aes(x = 23, y = 13, xend = 35, yend = 15),
               arrow = arrow(length = unit(0.5, "cm"))) +
  annotate(geom="text", x = 10, y = 11.5, label="Primate Unique\nSNP Positions",
           color="black", size = 2)

#add data on PhyloP and TTS distance

#for all causative SNP positions
cCRE_phyloP <- as.numeric(system(paste(" awk '{print $5}' ~/Polygenic_Inference/Iluvatar_Uploads/mammals_causative_cCRE2_RBC_hg38.bed", sep = ""), intern = T))
NOTcCRE_phyloP <- as.numeric(system(paste(" awk '{print $5}' ~/Polygenic_Inference/Iluvatar_Uploads/mammals_causative_NOTcCRE2_RBC_hg38.bed", sep = ""), intern = T))
...
phylop_plot <- ggplot(cCRE_plotting_df_phyloP, aes(x = type, y = PhyloP)) + geom_boxplot(fill = "gray", linewidth = 0.1, outlier.size = 0.0001) + 
  ...
  theme(axis.line = element_line(colour = "black"), 
        ...
        text = element_text(family = "Arial"), 
        plot.margin = unit(c(0.2, 0.2, 0.2, 0.5), "cm")) +
  annotate(geom="text", x=1.5, y=7.5, label=paste("Wilcoxon-rank sum, P < ", signif(wilcox_corr$p.value, digits = 3), sep = ""),
           color="black", size =6/.pt) + scale_y_continuous(limits = c(-3, 8))

#wilcoxon test for reduced TSS distance
#for all causative SNP positions

TSS_cCRE <- as.numeric(readLines("~/Polygenic_Inference/Iluvatar_Uploads/TSS_cCRE2_RBC.txt"))
TSS_NOTcCRE <- as.numeric(readLines("~/Polygenic_Inference/Iluvatar_Uploads/TSS_NOTcCRE2_RBC.txt"))
...
TSS_plot <- ggplot(cCRE_plotting_df, aes(x = type, y = distance/1000)) + geom_boxplot(fill = "gray", linewidth = 0.1, outlier.size = 0.0001) + 
  ...
  theme(axis.line = element_line(colour = "black"), 
        ...
        axis.text.x = element_text(size = 6),
        plot.margin = unit(c(0.2, 0.2, 0.2, 0.5), "cm")) +
  annotate(geom="text", x=1.5, y=max(TSS_NOTcCRE/1000) + 30, label=paste("Wilcoxon-rank sum, \n P < ", signif(wilcox_corr_TSS$p.value, digits = 3), sep = ""),
           color="black", size =6/.pt) + scale_y_continuous(limits = c(0, 240))

figure2_cow <- plot_grid(PGLS_plot_mam, PGLS_plot_primates, monoPlot, TSS_plot, phylop_plot, CC_plot, nrow = 3, ncol =2, labels = "AUTO", label_size = 8, align = "v")


#save plot
ggsave(filename = "Figure_2_RBC.png", device = "png", plot = figure2_cow, path = "~/Polygenic_Inference/Big_Loop_Output2", width = 11, height = 11, units = "cm", dpi = 1200)
ggsave(filename = "Figure_2_RBC.tiff", device = "tiff", plot = figure2, path = "~/Polygenic_Inference/Big_Loop_Output2", width = 11, height = 11, units = "cm", dpi = 1200)
